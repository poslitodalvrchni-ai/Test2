...
async def grant_achievement(user: discord.Member, ach_id: str):
    ...
    if ch:
        embed = discord.Embed(title="🏆 Achievement Unlocked!", description=f"{user.mention} Got achievement: **{ach_data['name']}**\n{ach_data['desc']}", color=discord.Color.gold())
        await ch.send(embed=embed)
    if ach_id != 'completionist':
        updated_doc = await db.users.find_one({'_id': user.id})
        count = len(updated_doc.get('achievements', []))
        if count >= 49 and 'completionist' not in updated_doc.get('achievements', []): await grant_achievement(user, 'completionist')
        if count >= 25 and 'halfway' not in updated_doc.get('achievements', []): await grant_achievement(user, 'halfway')
...
async def on_ready():
    ...
    if not hint_timer.is_running() and is_game_active: hint_timer.start()
    await bot.change_presence(activity=discord.Game(name="One hour one guess 🕰️ (!guess)"))

# --- HELP SYSTEM ---

def get_help_embed(perms):
    embed = discord.Embed(title="📚 Command List", color=discord.Color.blue())
    
    # PLAYER
    p_cmds = (
        "**`!guess`** - Guess the item (Standard prefix command)\n"
        "**`/mywins`** - Check your total wins\n"
        "**`/current`** - Shows all revealed hints for active game\n"
        "**`/nexthint`** - Shows the time until next hint\n"
        "**`/ach`** - Shows all your achievements"
    )
    embed.add_field(name=":video_game: Player Commands", value=p_cmds, inline=False)
    ...
    if perms['host']:
        h_cmds = (
            ...
            "**`/start`** - Start game (requires item + 5 hints)\n"
            "**`/revealhint`** - Manually reveal the next hint\n"
            "**`/queue`** - Show status of all 5 slots"
        )
        embed.add_field(name=":tools: Host Commands", value=h_cmds, inline=False)
        
    # ADMIN
    if perms['admin']:
        a_cmds = (
            f"**`/achgive`** - Give achievement to user {LOCK_EMOJI}\n"
            f"**`/achremove`** - Remove achievement from user {LOCK_EMOJI}\n"
            f"**`/reset`** - Reset specific user stats {LOCK_EMOJI}\n"
            f"**`/removewin`** - Remove wins from user {LOCK_EMOJI}\n"
            f"**`/fullreset`** - RESTART BOT STORAGE {LOCK_EMOJI}\n"
        ...
        d_cmds = (
            ...
            f"**`/testping`** - Test bots ping {LOCK_EMOJI}\n"
            "**`/reactiontest`** - Test reaction functionality"
        )
        embed.add_field(name=":beetle: Dev Commands", value=d_cmds, inline=False)

    embed.set_footer(text=f"One Hour One Guess • {LOCK_EMOJI} - do not use unless necessary")
    return embed

@bot.tree.command(name="help", description="Show available commands")
async def slash_help(interaction: discord.Interaction):
    perms = await check_permissions(interaction.user)
    embed = get_help_embed(perms)
    await interaction.response.send_message(embed=embed, ephemeral=True)

@bot.tree.command(name="poststaffhelp", description="Post the help embed to the staff channel")
async def slash_poststaffhelp(interaction: discord.Interaction):
    perms = await check_permissions(interaction.user)
    if not perms['admin']: return await interaction.response.send_message("❌ Admin only.", ephemeral=True)
...
async def prefix_guess(ctx, *, guess: str):
    ...
    if not is_game_active:
        if last_winner_time and (now - last_winner_time).total_seconds() <= 7.5: await grant_achievement(ctx.author, 'too_late')
    ...
    if g_low == correct_answer.lower():
        ...
        try:
            ...
            if target_rid:
                ...
                if role and role not in ctx.author.roles: await ctx.author.add_roles(role)
        except: pass
...
async def slash_mywins(interaction: discord.Interaction):
    ...
    if c > 0:
        ...
        if top_user and top_user[0]['_id'] == interaction.user.id: await grant_achievement(interaction.user, 'wealthy')
...
async def slash_current(interaction: discord.Interaction):
    if not is_game_active: return await interaction.response.send_message("No active game.", ephemeral=True)
...
async def slash_nexthint(interaction: discord.Interaction):
    ...
    else: await interaction.response.send_message("Timer not active.", ephemeral=True)

# --- ADMIN / STAFF COMMANDS (SLASH) ---
...
async def slash_setrole(interaction: discord.Interaction, type: app_commands.Choice[str], role: discord.Role):
    if not interaction.user.guild_permissions.administrator: return await interaction.response.send_message("❌ Admin permissions required.", ephemeral=True)
...
async def slash_start(interaction: discord.Interaction, game_id: int):
    ...
    if is_game_active: return await interaction.response.send_message("❌ Game active.", ephemeral=True)
    s_id = str(game_id)
    if s_id not in game_queue or not game_queue[s_id]['item']: return await interaction.response.send_message("❌ Slot empty.", ephemeral=True)
    data = game_queue[s_id]
    if len(data.get('hints', {})) != CONFIG['REQUIRED_HINTS']: return await interaction.response.send_message("❌ Missing hints.", ephemeral=True)
...
@bot.tree.command(name="stop", description="Stop active game or clear a specific slot")
async def slash_stop(interaction: discord.Interaction, game_id: int = None):
    perms = await check_permissions(interaction.user)
    if not perms['host']: return await interaction.response.send_message("❌ Host required.", ephemeral=True)
    ...
    if game_id is None:
        if is_game_active:
            is_game_active = False
            hint_timer.stop()
            await interaction.response.send_message("🛑 Stopped active game.")
            await save_game_state_to_db()
        else: await interaction.response.send_message("No active game to stop.", ephemeral=True)
    else:
        game_queue[str(game_id)] = {'item': None, 'hints': {}}
        await interaction.response.send_message(f"🗑️ Cleared slot #{game_id}")
        await save_game_state_to_db()

@bot.tree.command(name="stopall", description="Stop game and clear all slots")
async def slash_stopall(interaction: discord.Interaction):
    perms = await check_permissions(interaction.user)
    if not perms['host']: return await interaction.response.send_message("❌ Host required.", ephemeral=True)
    ...
    game_queue = {str(i): {'item': None, 'hints': {}} for i in range(1, 6)}
    await save_game_state_to_db()
    await interaction.response.send_message("🚨 All stopped and cleared.")

@bot.tree.command(name="setitem", description="Set secret item for a slot")
async def slash_setitem(interaction: discord.Interaction, game_id: int, item: str):
    perms = await check_permissions(interaction.user)
    if not perms['host']: return await interaction.response.send_message("❌ Host required.", ephemeral=True)
    
    if 1<=game_id<=5:
        game_queue[str(game_id)]['item'] = item.strip()
        await save_game_state_to_db()
        await interaction.response.send_message(f"✅ Slot {game_id} item: {item}")
    else: await interaction.response.send_message("❌ Slot 1-5 only.", ephemeral=True)

@bot.tree.command(name="sethint", description="Set a specific hint for a slot")
async def slash_sethint(interaction: discord.Interaction, game_id: int, hint_number: int, text: str):
    perms = await check_permissions(interaction.user)
    if not perms['host']: return await interaction.response.send_message("❌ Host required.", ephemeral=True)
    
    if 1<=game_id<=5 and 1<=hint_number<=7:
        if 'hints' not in game_queue[str(game_id)]: game_queue[str(game_id)]['hints'] = {}
        game_queue[str(game_id)]['hints'][str(hint_number)] = text.strip()
        await save_game_state_to_db()
...
async def slash_sethinttiming(interaction: discord.Interaction, minutes: int):
    ...
    await save_game_state_to_db()
    await interaction.response.send_message(f"✅ Timing set to {minutes}m.")

@bot.tree.command(name="revealhint", description="Manually reveal the next hint")
async def slash_revealhint(interaction: discord.Interaction):
    perms = await check_permissions(interaction.user)
    if not perms['host']: return await interaction.response.send_message("❌ Host required.", ephemeral=True)
    
    global current_hints_revealed, last_hint_reveal_time
    if not is_game_active: return await interaction.response.send_message("No game active.", ephemeral=True)
    nxt = len(current_hints_revealed) + 1
    if nxt <= CONFIG['REQUIRED_HINTS']:
         ...
         await save_game_state_to_db()
         await interaction.response.send_message(f"✅ Hint {nxt} revealed.")
    else: await interaction.response.send_message("All hints already revealed.", ephemeral=True)
...
# --- DEV COMMANDS (SLASH) ---
@bot.tree.command(name="status", description="See current game status")
async def slash_status(interaction: discord.Interaction):
    perms = await check_permissions(interaction.user)
    if not perms['admin']: return await interaction.response.send_message("❌ Dev required.", ephemeral=True)
    
    if is_game_active:
        await interaction.response.send_message(f"🟢 Active Game #{current_game_id}. Hints: {len(current_hints_revealed)}/7.")
    else: await interaction.response.send_message("🔴 No game active.")

@bot.tree.command(name="testping", description="Test bot latency")
async def slash_testping(interaction: discord.Interaction):
    perms = await check_permissions(interaction.user)
    if not perms['admin']: return await interaction.response.send_message("❌ Dev required.", ephemeral=True)
    await interaction.response.send_message(f"Pong! {round(bot.latency * 1000)}ms")

@bot.tree.command(name="reactiontest", description="Test reaction functionality")
async def slash_reactiontest(interaction: discord.Interaction):
    perms = await check_permissions(interaction.user)
    if not perms['admin']: return await interaction.response.send_message("❌ Dev required.", ephemeral=True)
    await interaction.response.send_message("Testing reactions...")
